...
void CMeshDataManager::ShutDown(void)
{
	// Release stored geometry
	for(std::map<std::string, MeshData*>::iterator ita = m_mMeshData.begin();
		ita != m_mMeshData.end(); ita++)
	{
		delete ita->second;
		ita->second = NULL;
	}
...
MeshData* CMeshDataManager::CreateTriangle(const std::string& geometryID, float size, EVertexType vertexType,
	const DirectX::XMFLOAT4& colour)
{
	...
	m_mMeshData.insert(std::pair<std::string, MeshData*>(geometryID, pNewGeometry));
...
MeshData* CMeshDataManager::CreateTriangleTriColour(const std::string& geometryID, float size, EVertexType vertexType,
		const DirectX::XMFLOAT4& colour)
{
	// Create mesh data
	MeshData* pNewGeometry = new MeshData(vertexType, 3, 3);
...
MeshData* CMeshDataManager::CreateCube(const std::string& geometryID, float size, float tessellation, EVertexType vertexType,
	const DirectX::XMFLOAT4& colour)
{
	...
	*/
	...
	m_mMeshData.insert(std::pair<std::string, MeshData*>("geometryID", pMeshData));
...
MeshData* CMeshDataManager::CreatePlane(const std::string& geometryID, float size, float tessellation, EVertexType vertexType,
		const DirectX::XMFLOAT4& colour)
{
	// Total number of vertices of the cube
...
MeshData* CMeshDataManager::CreateSinPlane(const std::string& geometryID, float size, float tessellation, EVertexType vertexType,
		const DirectX::XMFLOAT4& colour)
{
	// Total number of vertices of the cube
	int totalVerts = (tessellation + 1) * (tessellation + 1);
